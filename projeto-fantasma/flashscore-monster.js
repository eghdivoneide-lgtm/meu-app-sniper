  ...
  async extrairPartida(url) {
    ...
    try {
      ...
      const cabecalho = await page.evaluate(() => {
        ...
        const nomesRaw = Array.from(document.querySelectorAll('.participant__participantName, .duelParticipant__home, .duelParticipant__away')).map(n => n.innerText.trim()).filter(n => n.length > 0);
        const nomesUnicos = [...new Set(nomesRaw)];
        if(nomesUnicos.length >= 2) {
           mandante = nomesUnicos[0];
           visitante = nomesUnicos[1];
        } else if(title.includes('-')) {
            const parts = title.split('-');
            mandante = parts[0].replace(/[0-9\\s]+$/, '').trim();
            visitante = parts[1].split('placar')[0].split('Live')[0].split('H2H')[0].replace(/^[0-9\\s]+/, '').trim();
        }
      ...
      const clickTabAndWait = async (searchText1, searchText2) => {
         await page.evaluate((s1, s2) => {
            const links = document.querySelectorAll('button, a, div, span');
            const target = Array.from(links).find(el => el.innerText && (el.innerText.toUpperCase() === s1 || el.innerText.toUpperCase() === s2));
      ...
      const forms = await page.evaluate(() => {
         ...
         return { m: allForms[0] || "Desconhecida", v: allForms[1] || allForms[0] || "Desconhecida" };
      });
