...
(async () => {
  ...
  if (!fs.existsSync('match_ids_br.json')) {
    console.log('Erro: match_ids_br.json não encontrado. Rode o caca_ids_br.js primeiro.');
    return;
  }

  const matchIds = JSON.parse(fs.readFileSync('match_ids_br.json', 'utf-8'));
  console.log(`Carregou ${matchIds.length} jogos do Brasileirão para minerar em MODO 100% ASSUSTADOR.`);
  ...
  fs.writeFileSync('raw_br.json', arquivoSaida, 'utf-8');

  await browser.close();
  console.log('✅ Interceptação 100% Concluída (MODO ASSUSTADOR)! Arquivo raw_br.json salvo com sucesso! Em seguida o formatador gerará o Brasileirão 2026 oficial.');
})();
