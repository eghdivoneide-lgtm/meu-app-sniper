...
async function mineIds() {
  console.log('🕵️‍♂️ CAÇADOR DE IDs: Iniciando rastreamento no FlashScore (USL Championship)...');
  
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  // Vamos acessar a versão Global do Flashscore direcionada para a USL
  await page.goto('https://www.flashscore.com/football/usa/usl-championship/results/', { waitUntil: 'load' });
  ...
  console.log(`✅ Rastreados com sucesso: ${matchIds.length} jogos da USL Championship.`);
  
  fs.writeFileSync('match_ids_usl.json', JSON.stringify(matchIds, null, 2));
  console.log(`Arquivo 'match_ids_usl.json' foi salvo perfeitamente com os IDs extraídos!`);
