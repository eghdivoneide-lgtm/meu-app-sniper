...
(async () => {
  console.log('📡 Iniciando o Interceptador de Rede (Network Scraper) da MLS...');

  if (!fs.existsSync('match_ids.json')) {
    console.log('Erro: match_ids.json não encontrado. Rode o extrator.js primeiro.');
    return;
  }

  const matchIds = JSON.parse(fs.readFileSync('match_ids.json', 'utf-8'));
  console.log(`Carregou ${matchIds.length} jogos para minerar.`);
  ...
  for (let i = 0; i < matchIds.length; i++) {
    ...
    try {
      ...
      // Clicamos nav aba HT disparar a renderização do React
      try {
         const abaHT = await page.getByRole('link', { name: /1st Half|1º Tempo/i }).first();
         if (abaHT) await abaHT.click({timeout: 2000});
      } catch(e){}

      await delay(1500); // Aguarda renderizar as estatísticas na tela DOM
  ...
  const arquivoSaida = `const DADOS_MLS = ${JSON.stringify(resultadosDaLiga, null, 2)};\n\nexport default DADOS_MLS;`;
  fs.writeFileSync('../data/mls2025.js', arquivoSaida, 'utf-8');

  await browser.close();
  console.log('✅ Interceptação 100% Concluída! Arquivo data/mls2025.js atualizado brilhantemente!');
})();
