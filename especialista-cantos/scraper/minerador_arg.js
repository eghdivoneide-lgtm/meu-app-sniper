...
(async () => {
  console.log('📡 Iniciando o Interceptador de Rede (Network Scraper) da Argentina...');

  if (!fs.existsSync('match_ids_arg.json')) {
    console.log('Erro: match_ids_arg.json não encontrado. Rode o caca_ids_arg.js primeiro.');
    return;
  }

  const matchIds = JSON.parse(fs.readFileSync('match_ids_arg.json', 'utf-8'));
  console.log(`Carregou ${matchIds.length} jogos da Liga Argentina para minerar em MODO 100% ASSUSTADOR.`);
  ...
  fs.writeFileSync('raw_arg.json', arquivoSaida, 'utf-8');

  await browser.close();
  console.log('✅ Interceptação 100% Concluída! Arquivo raw_arg.json salvo com sucesso! Em seguida o formatador gerará o Argentina 2026 oficial.');
})();
