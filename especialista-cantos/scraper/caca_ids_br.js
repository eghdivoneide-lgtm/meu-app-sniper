...
async function mineIds() {
  console.log('🕵️‍♂️ CAÇADOR DE IDs: Iniciando rastreamento no FlashScore (Brasileirão Série A)...');
  
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  // Vamos acessar a versão Global do Flashscore direcionada para o Brasileirão Série A (Resultados)
  await page.goto('https://www.flashscore.com/football/brazil/serie-a/results/', { waitUntil: 'load' });
  ...
  console.log(`✅ Rastreados com sucesso: ${matchIds.length} jogos do Brasileirão.`);
  
  fs.writeFileSync('match_ids_br.json', JSON.stringify(matchIds, null, 2));
  console.log(`Arquivo 'match_ids_br.json' foi salvo perfeitamente com os IDs extraídos!`);
