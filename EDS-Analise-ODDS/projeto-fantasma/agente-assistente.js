...
function carregarDados(liga) {
  ...
  const window = {};
  const varNames = ['DADOS_USL','DADOS_MLS','DADOS_BR','DADOS_ARG','DADOS_BUN','DADOS_ECU','DADOS_CHI','DADOS_ARG_B'];
...
async function extrairIdsResultados(page, url, maxJogos = 80) {
  ...
  // Clicar "Mostrar mais" até carregar tudo (até 10x para ligas grandes)
  for (let i = 0; i < 10; i++) {
...
async function completarLiga(codigoLiga) {
  ...
  // Iniciar FlashscoreMonster (cria o browser internamente)
  const monster = new FlashscoreMonster();
  await monster.iniciar();

  // Passo 1: Obter IDs da página de resultados usando o browser do Monster
  log(`Acessando resultados: ${liga.url_results}`, 'flashscore');
  const pageScan = await monster.browser.newPage();
  await pageScan.setUserAgent(
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36'
  );

  const idsFlashscore = await extrairIdsResultados(pageScan, liga.url_results, 200);
  await pageScan.close();

  log(`IDs encontrados no FlashScore: ${idsFlashscore.length}`, 'info');

  // Passo 2: Filtrar novos e extras
  const idsFlashscoreSet = new Set(idsFlashscore);
  const idsNovos = idsFlashscore.filter(id => !idsExistentes.has(id));
  const idsExtras = [...idsExistentes].filter(id => !idsFlashscoreSet.has(id));
  ...
  console.log(`  🗑️  EXTRAS (remover): ${idsExtras.length}`);
  console.log('═══════════════════════════════════════════════════');

  // Remover jogos extras (que não existem no FlashScore da liga)
  if (idsExtras.length > 0) {
    const before = dados.jogos.length;
    const extrasSet = new Set(idsExtras);
    const removidos = dados.jogos.filter(j => extrasSet.has(j.id || j.match_id));
    dados.jogos = dados.jogos.filter(j => !extrasSet.has(j.id || j.match_id));
    log(`Removidos ${before - dados.jogos.length} jogos que não pertencem à liga:`, 'fix');
    removidos.forEach(j => log(`  ❌ ${j.mandante} vs ${j.visitante} | ${j.data || '?'}`, 'warn'));
  }

  if (idsNovos.length === 0) {
    if (idsExtras.length > 0) {
      // Salvamos as remoções mesmo sem novos jogos
    ...
    await monster.fechar();
    log(`Base ajustada! ${dados.jogos.length} jogos (${idsExtras.length} removidos).`, 'success');
  ...
  for (let i = 0; i < idsNovos.length; i++) {
    ...
    while (tentativas < maxRetries && !dadosJogo) {
      try {
        dadosJogo = await monster.extrairPartida(matchUrl, {
          liga: liga.nome,
...
(async () => {
  ...
  for (const liga of ligasParaProcessar) {
    try {
      const result = await completarLiga(liga);
