...
const LIGAS = {
  ...
  ARG_B: { arquivo: 'argentina_b2026.js', variavel: 'DADOS_ARG_B', nome: 'Primera B Nacional (Argentina)' },
  // 7 ligas novas (29-30/04/2026):
  BR_B:    { arquivo: 'brasileiraoB2026.js',  variavel: 'DADOS_BR_B',    nome: 'Brasileirão Série B' },
  J2:      { arquivo: 'j2league2026.js',      variavel: 'DADOS_J2',      nome: 'J2 League (Japão)' },
  J2_J3:   { arquivo: 'j2j3league2026.js',    variavel: 'DADOS_J2_J3',   nome: 'J2/J3 League (Japão — 4 federações)' },
  ARG_M:   { arquivo: 'metropolitana2026.js', variavel: 'DADOS_ARG_M',   nome: 'Primera B Metropolitana (Argentina)' },
  CHN_1:   { arquivo: 'chinaone2026.js',      variavel: 'DADOS_CHN_1',   nome: 'China League One' },
  CHN_2:   { arquivo: 'chinatwo2026.js',      variavel: 'DADOS_CHN_2',   nome: 'China League Two' },
  CHN_SUP: { arquivo: 'chinasuper2026.js',    variavel: 'DADOS_CHN_SUP', nome: 'Chinese Super League' }
};
...
function carregarDados(liga) {
  ...
  const window = {};
  const varNames = ['DADOS_ARG','DADOS_ARG_B','DADOS_USL','DADOS_MLS','DADOS_BR','DADOS_BUN','DADOS_ECU',
                    'DADOS_BR_B','DADOS_J2','DADOS_J2_J3','DADOS_ARG_M','DADOS_CHN_1','DADOS_CHN_2','DADOS_CHN_SUP'];
...
function recalcularLiga(codigoLiga) {
  ...
  // Salvar como .js APENAS no caminho oficial do Yaaken-Scanner (EDS-Analise-ODDS)
  ...
  const yaakDirOficial = path.join(__dirname, '..', 'Yaaken-Scanner', 'yaaken-data');
  if (!fs.existsSync(yaakDirOficial)) {
    console.log(`  ❌ Path oficial Yaaken-Scanner não encontrado: ${yaakDirOficial}`);
  } else {
    fs.writeFileSync(path.join(yaakDirOficial, `escoteiro_${codigoLiga}.js`), jsContent);
    console.log(`  📂 Yaaken-Scanner (oficial) atualizado → escoteiro_${codigoLiga}.js`);
  }
