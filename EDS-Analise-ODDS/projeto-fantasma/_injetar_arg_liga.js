/**
 * INJETOR CIRÚRGICO — Liga Profesional Argentina — UPSERT (3 caminhos)
 */
const fs = require('fs');
const path = require('path');

const RAW = path.join(__dirname, 'arg_rodada_2_2026-04-22.json');
const ALVOS = [
  'C:/Users/egnal/OneDrive/Área de Trabalho/MEU APP/EDS-Analise-ODDS/especialista-cantos/data/argentina2026.js',
  'C:/Users/egnal/OneDrive/Área de Trabalho/MEU APP/especialista-cantos/data/argentina2026.js',
  'C:/Users/egnal/OneDrive/Área de Trabalho/MEU APP/Yaaken-Scanner/yaaken-data/argentina2026.js'
];
...
for (const alvo of ALVOS) {
  ...
  fn(sandbox.window);
  const dados = sandbox.window.DADOS_ARG;
  if (!dados || !Array.isArray(dados.jogos)) {
    console.log(`ERRO: DADOS_ARG.jogos não encontrado em ${alvo}`);
    continue;
  }
  ...
  const out =
    `// Liga Profesional Argentina 2026 — Injeção Varredor v4 (upsert)\n` +
    `// ${dados.jogos.length} jogos | Atualizado: ${dados.ultimaAtualizacao}\n` +
    `window.DADOS_ARG = ${JSON.stringify(dados, null, 2)};\n`;
